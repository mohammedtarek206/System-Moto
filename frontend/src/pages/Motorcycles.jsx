...
import {
  Plus, Search, Edit, Trash2, X, Bike, Filter,
  ChevronDown, Image, QrCode, Barcode, Package
} from 'lucide-react';
...
const BRANDS = ['Honda', 'Yamaha', 'Suzuki', 'Kawasaki', 'KTM', 'Bajaj', 'TVS', 'Hero', 'Royal Enfield', 'أخرى'];

function VehicleForm({ vehicle, onSave, onClose, token, productType = 'motorcycles' }) {
  const { isRTL } = useLang();
  const isEdit = !!vehicle;
  const typeLabel = productType === 'motorcycles' ? 'موتسيكل' : 'سكوتر';
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (isEdit) {
        await axios.put(`${API}/products/${vehicle._id}`, fd, {
        ...
        toast.success(`تم إضافة ${typeLabel} بنجاح ✅`);
      }
      onSave();
    } catch (err) {
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-black text-white flex items-center gap-2">
            ...
            {isEdit ? `تعديل ${typeLabel}` : `إضافة ${typeLabel} جديد`}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="p-4 rounded-xl bg-white/5 border border-white/10">
            <h3 className="text-sm font-bold text-orange-500 mb-3">البيانات الأساسية</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              <div className="col-span-2 md:col-span-3">
                <label className="block text-xs font-bold text-[var(--text-muted)] mb-1">الاسم <span className="text-[var(--text-muted)] font-normal">(اختياري)</span></label>
                <input name="name" value={form.name} onChange={handleChange} className="input-field" placeholder="مثال: Honda CB125" />
              </div>
              <div>
                <label className="block text-xs font-bold text-[var(--text-muted)] mb-1">الماركة *</label>
                <input list="brands-list" name="brand" value={form.brand} onChange={handleChange} className="input-field" placeholder="Honda" required />
              ...
              <div>
                ...
                <input name="model" value={form.model} onChange={handleChange} className="input-field" placeholder="CB125" required />
              </div>
              <div>
                <label className="block text-xs font-bold text-[var(--text-muted)] mb-1">سنة الصنع</label>
                <input type="number" name="year" value={form.year} onChange={handleChange} className="input-field" min="1990" max="2030" />
              </div>
              <div>
                <label className="block text-xs font-bold text-[var(--text-muted)] mb-1">اللون</label>
                <input name="color" value={form.color} onChange={handleChange} className="input-field" placeholder="أبيض" />
              </div>
              <div>
                <label className="block text-xs font-bold text-[var(--text-muted)] mb-1">السعة (CC)</label>
                <input type="number" name="engineCC" value={form.engineCC} onChange={handleChange} className="input-field" placeholder="125" />
              </div>
              <div>
          ...
          {/* Identifiers */}
          <div className="p-4 rounded-xl bg-white/5 border border-white/10">
            <h3 className="text-sm font-bold text-orange-500 mb-3">أرقام التعريف</h3>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-bold text-[var(--text-muted)] mb-1">رقم الشاسيه</label>
                <input name="chassisNo" value={form.chassisNo} onChange={handleChange} className="input-field" placeholder="VIN/رقم الشاسيه" />
              </div>
              <div>
                <label className="block text-xs font-bold text-[var(--text-muted)] mb-1">رقم الموتور</label>
                <input name="engineNo" value={form.engineNo} onChange={handleChange} className="input-field" placeholder="رقم الموتور" />
              </div>
              <div>
                <label className="block text-xs font-bold text-[var(--text-muted)] mb-1">رمز المنتج (SKU)</label>
                <input name="sku" value={form.sku} onChange={handleChange} className="input-field" placeholder="سيتم إنشاؤه تلقائياً" />
              </div>
              <div>
                <label className="block text-xs font-bold text-[var(--text-muted)] mb-1">الباركود</label>
                <input name="barcode" value={form.barcode} onChange={handleChange} className="input-field" placeholder="سيتم إنشاؤه تلقائياً" />
              </div>
            </div>
          </div>
          ...
          {/* Pricing */}
          <div className="p-4 rounded-xl bg-white/5 border border-white/10">
            <h3 className="text-sm font-bold text-orange-500 mb-3">الأسعار والكمية</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div>
                <label className="block text-xs font-bold text-[var(--text-muted)] mb-1">سعر الشراء *</label>
                <input type="number" name="buyPrice" value={form.buyPrice} onChange={handleChange} className="input-field" placeholder="0" required min="0" />
              </div>
              <div>
                <label className="block text-xs font-bold text-[var(--text-muted)] mb-1">سعر البيع *</label>
                <input type="number" name="sellPrice" value={form.sellPrice} onChange={handleChange} className="input-field" placeholder="0" required min="0" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs font-bold text-[var(--text-muted)] mb-1">الحد الأدنى</label>
                <input type="number" name="minQuantity" value={form.minQuantity} onChange={handleChange} className="input-field" min="0" />
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 rounded-xl bg-white/5 border border-white/10">
              <h3 className="text-sm font-bold text-orange-500 mb-3">الوصف</h3>
              <textarea name="description" value={form.description} onChange={handleChange} className="input-field h-24 resize-none" placeholder="وصف تفصيلي..." />
            </div>
            <div className="p-4 rounded-xl bg-white/5 border border-white/10">
              <h3 className="text-sm font-bold text-orange-500 mb-3">الصورة</h3>
              <label className="flex flex-col items-center justify-center h-24 border-2 border-dashed border-white/20 rounded-xl cursor-pointer hover:border-orange-500/50 transition-colors">
                {imagePreview ? (
                  <img src={imagePreview} alt="preview" className="h-full w-full object-contain rounded-xl" />
                ) : (
                  <>
                    <Image size={24} className="text-[var(--text-muted)] mb-1" />
                    <span className="text-xs text-[var(--text-muted)]">اختر صورة</span>
                  </>
                )}
...
export default function Motorcycles() {
  ...
  const fetchVehicles = useCallback(async () => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ product_type: 'motorcycles' });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[var(--text-muted)] text-sm mt-1">إدارة مخزون الموتسيكلات</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'إجمالي الموتسيكلات', value: totalCount, color: 'from-orange-500 to-red-500' },
          { label: 'متاح بالمخزن', value: availableCount, color: 'from-green-500 to-emerald-600' },
          { label: 'جديد', value: newCount, color: 'from-blue-500 to-indigo-600' },
          { label: 'قيمة المخزون', value: `${totalValue.toLocaleString('ar-EG')} ج`, color: 'from-purple-500 to-violet-600' },
        ].map((s, i) => (
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="ابحث بالماركة أو الموديل أو الشاسيه..."
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-16">
            <Bike size={48} className="mx-auto text-[var(--text-muted)] mb-4 opacity-30" />
            <p className="text-[var(--text-muted)]">لا توجد موتسيكلات</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/10">
                  {['#', 'الصورة', 'الماركة / الموديل', 'السنة', 'الحالة', 'الشاسيه', 'الكمية', 'سعر الشراء', 'سعر البيع', 'إجراءات'].map(h => (
                    <th key={h} className="px-3 py-3 text-start text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {vehicles.map((v, i) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-3">
                      {v.image ? (
                        ...
                        <div className="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center">
                          <Bike size={16} className="text-[var(--text-muted)]" />
                        </div>
                      )}
                    </td>
                    <td className="px-3 py-3">
                    ...
                    <td className="px-3 py-3 text-[var(--text-muted)] text-xs font-mono">{v.chassisNo || '-'}</td>
                    <td className="px-3 py-3">
                      <span className={`badge ${v.quantity > 0 ? 'badge-success' : 'badge-danger'}`}>
                        {v.quantity}
                      </span>
                    </td>
                    <td className="px-3 py-3 text-[var(--text-secondary)]">{(v.buyPrice || 0).toLocaleString('ar-EG')} ج</td>
                    <td className="px-3 py-3 font-bold text-orange-400">{(v.sellPrice || 0).toLocaleString('ar-EG')} ج</td>
                    <td className="px-3 py-3">
                      <div className="flex items-center gap-2">
                        <button onClick={() => setShowQR(v)} title="QR" className="p-1.5 rounded-lg hover:bg-white/10 text-purple-400">
                          <QrCode size={16} />
                        </button>
                        <button onClick={() => { setEditVehicle(v); setShowForm(true); }} className="p-1.5 rounded-lg hover:bg-white/10 text-blue-400">
                          <Edit size={16} />
                        </button>
                        <button onClick={() => setDeleteConfirm(v._id)} className="p-1.5 rounded-lg hover:bg-white/10 text-red-400">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </td>
      ...
      <AnimatePresence>
        {deleteConfirm && (
          <motion.div
            ...
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          >
            <motion.div className="card p-6 max-w-sm w-full text-center"
            ...
            >
              ...
              <p className="text-[var(--text-muted)] text-sm mb-6">هل أنت متأكد من حذف هذا الموتسيكل؟</p>
