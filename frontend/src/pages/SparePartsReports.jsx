...
export default function SparePartsReports() {
  ...
  const totals = {
    saleCount: data.reduce((a, r) => a + r.saleCount, 0),
    totalSold: data.reduce((a, r) => a + r.totalSold, 0),
    totalRevenue: data.reduce((a, r) => a + r.totalRevenue, 0),
    totalProfit: data.reduce((a, r) => a + r.totalProfit, 0),
    currentStock: data.reduce((a, r) => a + r.currentStock, 0),
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-white flex items-center gap-2">
            <Package className="text-orange-500" size={28}/> تقارير قطع الغيار
          </h1>
          <p className="text-[var(--text-muted)] text-sm mt-1">تقرير تفصيلي لمبيعات قطع الغيار والإكسسوارات</p>
        </div>
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => exportToPDF({ title:'تقرير قطع الغيار', columns:COLS, rows:displayData, filename:'spare-parts-report' })} className="btn-secondary flex items-center gap-1 text-sm"><FileText size={14}/> PDF</button>
          <button onClick={() => exportToExcel({ title:'تقرير قطع الغيار', columns:COLS, rows:displayData, filename:'spare-parts-report' })} className="btn-secondary flex items-center gap-1 text-sm"><Download size={14}/> Excel</button>
          <button onClick={() => exportToCSV({ columns:COLS, rows:displayData, filename:'spare-parts-report' })} className="btn-secondary flex items-center gap-1 text-sm">CSV</button>
          <button onClick={() => printTable({ title:'تقرير قطع الغيار', columns:COLS, rows:displayData })} className="btn-secondary flex items-center gap-1 text-sm"><Printer size={14}/> طباعة</button>
        </div>
      </div>
      ...
      <div className="card">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select value={sortBy} onChange={e=>setSortBy(e.target.value)} className="input-field w-36 text-sm">
          ...
          <div className="relative ms-auto">
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="بحث عن قطعة..." className="input-field ps-8 text-sm w-48"/>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label:'عدد مرات البيع', value:`${totals.saleCount.toLocaleString('ar-EG')} مرة`, icon:Package, color:'from-orange-500 to-amber-600' },
          { label:'إجمالي المبيعات', value:formatCurrency(totals.totalRevenue), icon:TrendingUp, color:'from-blue-500 to-indigo-600' },
          { label:'إجمالي الأرباح', value:formatCurrency(totals.totalProfit), icon:TrendingUp, color:'from-green-500 to-emerald-600' },
          { label:'الكمية الحالية بالخزن', value:`${totals.currentStock.toLocaleString('ar-EG')} قطعة`, icon:Package, color:'from-purple-500 to-violet-600' },
        ].map((s,i) => (
      ...
      <div className="card overflow-hidden">
        {loading ? (
          <div className="text-center py-12"><div className="animate-spin w-8 h-8 border-2 border-orange-500 border-t-transparent rounded-full mx-auto mb-3"/><p className="text-[var(--text-muted)]">جاري التحميل...</p></div>
        ) : data.length === 0 ? (
          <div className="text-center py-16"><Package size={48} className="mx-auto text-[var(--text-muted)] mb-4 opacity-30"/><p className="text-[var(--text-muted)]">لا توجد بيانات</p></div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/10">
                  {['#','اسم القطعة','عدد مرات البيع','الكمية المباعة','الإيرادات','الأرباح','متوسط السعر','الكمية بالمخزن','آخر عملية بيع'].map(h => (
              ...
              <tbody>
                {data.map((r,i) => (
                  <motion.tr key={r.id||i} initial={{opacity:0}} animate={{opacity:1}} transition={{delay:i*0.02}}
                    ...
                    <td className="px-3 py-3 text-center"><span className="badge badge-info">{r.saleCount}</span></td>
                    <td className="px-3 py-3 text-center"><span className="badge badge-success">{r.totalSold}</span></td>
              ...
              <tfoot>
                <tr className="border-t-2 border-white/20 bg-white/5 font-black">
                  ...
                  <td className="px-3 py-3 text-center text-white">{totals.saleCount}</td>
